...
function binRota(arr) {
  // store the result
  const rota = [];
  // loop through input array, append even-indexed subarrays and reverted odd-indexed subarrays to result
  for (let i = 0; i < arr.length; i++) {
    if (i % 2 === 0) rota.push(...arr[i]);
    else rota.push(...arr[i].reverse());
  }
  // return the result
  return rota;
}