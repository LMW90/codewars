...
function findBall(scales) {
  // assign ball indices to a variable
  let balls = [0,1,2,3,4,5,6,7];
  // untill there is one ball left, halve the balls parts on the scale and repeat
  while (balls.length > 1) {
    let [leftScale, rightScale] = [balls.slice(0, balls.length/2), balls.slice(balls.length/2)];
    balls = (scales.getWeight(leftScale, rightScale) < 0) ? leftScale : rightScale;
  }
  // return the last ball left
  return balls[0];
}