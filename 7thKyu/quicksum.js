...
function quicksum(packet){
  ...
  // return 0 if any character is not an upper case lettor or space
  if (characters.some(char => !" ABCDEFGHIJKLMNOPQRSTUVWXYZ".includes(char))) return 0;
  // calculate the checksum and return
  return packet.split('').reduce((checksum, char, index) => {
    return (char == ' ') ? checksum : checksum + (index + 1) * (char.charCodeAt() - 64);
  }, 0);
}