...
function madShout(sidewalk){
  ...
  // remove characters before "Y" and after "F"
  sidewalk = sidewalk.replace(/.*(?=Y)/, '').replace(/(?<=F).*/, '');
  // one "i" for each two dashes of distance
  let is = Math.ceil((sidewalk.length - 1 ) / 2);
  return `O${(is <= 1)? 'i' : 'i'.repeat(is)} F!`
};