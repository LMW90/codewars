...
const complete = str =>{
  for (let i = 0; i < str.length; i++){
    // keep adding bigger and bigger reversed slices of front until palindrom got got
    let temp = str + str.slice(0,i+1).split('').reverse().join('');
    if (temp == temp.split('').reverse().join('')) return temp;
  }
}