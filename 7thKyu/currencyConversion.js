...
function convertMyDollars(usd, currency) {
  // determine radix of currency rate
  let isDecimal = ('AEIOU'.includes(currency[0])) ? true : false;
  // calculate respective decimal conversion rate
  let rate = (isDecimal) ? CONVERSION_RATES[currency] : parseInt(CONVERSION_RATES[currency], 2);
  // return calculated amount converted
