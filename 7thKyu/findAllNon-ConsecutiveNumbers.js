...
function allNonConsecutive (arr) {
  ...
  let nonConsecutives = [];
  // add an object for each non-consecutive to result array
  for (let i = 1; i < arr.length; i++){
    if (arr[i] - arr[i-1] !== 1) nonConsecutives.push({ "i": i, "n": arr[i] });
  }
  // return result array or empty if all elements are non-consecutive
  return (nonConsecutives.length !== arr.length) ? nonConsecutives : [];
}