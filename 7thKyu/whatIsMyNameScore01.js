/* he 26 letters of the English alphabets are randomly divided into 5 groups of 5 letters with the remaining letter being ignored. Each of the group is assigned a score of more than 0. The ignored letter always has a score of 0.
...
function nameScore(name){
  // extract the keys from scores object
  let alphaKeys = Object.keys(alpha);
  // loop through characters and add up scores
  let nameScore = name.split("").reduce((runningPoints, character) => {
    character = character.toUpperCase();
    for (let key of alphaKeys){
      if (key.includes(character)) return runningPoints + alpha[key];
    }
    return runningPoints;
  }, 0)
  // return the result object
  return { [name]: nameScore };
}