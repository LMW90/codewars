...
function holidayCount (a1, a2){
  // find and convert to int a number that is not NaN
  const theirs = parseInt(a1.find(item => typeof item === "number" && !isNaN(item)));
  const yours = parseInt(a2.find(item => typeof item === "number" && !isNaN(item)));
  // if nothing found, return
  if (isNaN(yours) || isNaN(theirs)) return "Not possible";
  // return result of comparison
  return (yours > theirs) ? "Wrong" : (yours < theirs) ? "Right" : "Same";
}