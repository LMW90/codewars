...
function cleanedCounts(data) {
  ...
  let cleansedData = [data[0]];
  // populate result array
    for (let i = 1; i < data.length; i++){
      // if decrease in regard to previous (fixed) value, add increased value
      if (data[i] < cleansedData[i-1]) {
        cleansedData.push(data[i]+1);
        // if value not faulty append as is
      } else {
        cleansedData.push(data[i]);
      }
    }
  return cleansedData;
}