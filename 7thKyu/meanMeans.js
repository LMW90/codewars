...
function geo_mean(nums, arith_mean) {
  ...
  let missingNumber = arith_mean*(nums.length + 1) - nums.reduce((sum, num) => sum + num, 0);
  // calculate the geometric mean for nums with missing number restored
  return [...nums, missingNumber].reduce((product, num) => product * num, 1)**(1/(nums.length+1));
}