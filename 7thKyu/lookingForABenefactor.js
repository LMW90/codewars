...
*/
function newAvg(array, newAverage) {
  // read number of doantions and sum current donations
  let DonationCount = array.length;
  let totalDonations = array.reduce((sum, donation) => sum + donation, 0);
  // calculate the missing donation
  let requiredDonation = Math.ceil(newAverage * (DonationCount + 1) - totalDonations);
  // handling non-positive result
  if (requiredDonation <= 0) throw new Error("Expected New Average is too low");
  // return the result
