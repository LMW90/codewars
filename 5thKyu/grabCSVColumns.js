...
function csvColumns(csv, indices) {
  // split string into string rows, split those into arrays of items and filter out non-indice compliant
  let filteredRows = csv.split('\n')
    .map(row => row.split(',').filter((item, index) => indices.includes(index)));
  // if flattene array of rows has any items join then and return, else return empty string
	return (filteredRows.flat().length) ? filteredRows.join('\n') : '';
}