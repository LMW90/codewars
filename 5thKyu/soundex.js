...
var soundex = function(names) {
  const points = { b: 1, f: 1, p: 1, v: 1, c: 2, g: 2, j: 2, k: 2,
    q: 2, s: 2, x: 2, z: 2, d: 3, t: 3, l: 4, m: 5, n: 5, r: 6,
  };
  // split input into words
  let words = names.split(' ');
  // save first characters
  let firstCharacters = words.map(word => word[0]);
  console.log(firstCharacters)
  // remove non-leading "h"s and "w"s
  words = words.map(word => word[0] + word.slice(1).replace(/[hw]/ig, ''));
  // replace consonants with point values
  words = words.map(word => word.replace(/[bfpvcgjkqsxzdtlmnr]/ig, char => points[char.toLowerCase()]));
  // replace adjacent duplicate digits
  words = words.map(word => word.replace(/(\d)\1+/g, '$1'));
  // remove non-leading vowels
  words = words.map(word => word[0] + word.slice(1).replace(/[aeiouy]/g, ''));
  // replace first character with original first character
  words = words.map((word, index) => firstCharacters[index].toUpperCase() + word.slice(1));
  // append three zeros
  words = words.map(word => word + '000');
  // trim words to 4 characters each
  words = words.map(word => word.slice(0,4));
  // return a string
  return words.join(' ');
}