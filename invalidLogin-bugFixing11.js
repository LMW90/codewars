...
function validate(username, password){
  var database = new Database();
  // regexp with anon capture group for exactly two || or two //
  const re = /(?:\/{2}|\|{2})/
  // test user name and password for regexp
  if (re.test(username) || re.test(password)) return "Wrong username or password!"
