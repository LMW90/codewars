...
*/
function rot(str) {
  // split into substrings, reverse the substrings, reverse substrings order and join
  return str.split('\n').map(substring => substring.split('').reverse().join('')).reverse().join('\n');
}
function selfieAndRot(str) {
  // split into substrings, append each substring with dots, and join
  let selfie = str.split('\n').map(substring => substring + '.'.repeat(substring.length)).join('\n');
  // return above string with appended return value of rot function called no above string;
  return selfie + '\n' + rot(selfie);
}
