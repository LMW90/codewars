...
function streetFighterSelection(fighters, position, moves){
  // assign input position to variables
  let [row, column] = [...position];
  // store trailed fighters in a variable
  let result = [];
  // loop through moves change position according to instructions and append to result traced fighter
  for (let move of moves){
    if (move == "up" && row > 0) row--;
    if (move == "down" && row < fighters.length - 1) row++;
    if (move == "left"){
      if (column == 0) column = fighters[0].length - 1;
      else column--;
    }
    if (move == "right"){
      if (column == fighters[0].length - 1) column = 0;
      else column++;
    }
