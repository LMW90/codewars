...
function isLanguageDiverse(list) {
  // vars to store count
  let [rCount, pCount, jsCount] = [0, 0, 0];
  // increment if language encountered
  for (let dev of list){
    if (dev.language === "JavaScript") jsCount++;
    if (dev.language === "Python") pCount++;
    if (dev.language === "Ruby") rCount++;
  }
  // check if max not more than twice as big as min
  return Math.max(jsCount, pCount, rCount) <= Math.min(jsCount, pCount, rCount) * 2;
}